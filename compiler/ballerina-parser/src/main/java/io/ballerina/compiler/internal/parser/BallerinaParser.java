...
public class BallerinaParser extends AbstractParser {
    ...
    private STNode parseExpressionRhsInternal(OperatorPrecedence currentPrecedenceLevel, STNode lhsExpr,
                                              ...
                                              boolean isInConditionalExpr) {
        STToken nextToken = peek();
        if (isAction(lhsExpr) || isEndOfActionOrExpression(nextToken, isRhsExpr, isInMatchGuard)) {
    ...
    private STNode getExpressionAsStatement(STNode expression) {
        switch (expression.kind) {
            ...
            default:
                ...
                if (this.errorHandler.hasAncestorContext(ParserRuleContext.VAR_DECL_STMT) ||
                        this.errorHandler.hasAncestorContext(ParserRuleContext.DO_BLOCK)) {
                    return STNodeFactory.createExpressionStatementNode(expression.kind, expression, semicolon);
                }
